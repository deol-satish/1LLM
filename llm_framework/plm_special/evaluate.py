...
def evaluate_on_simulated_env(args, model, exp_pool, target_return, loss_fn ,llm_freq=100,process_reward_fn=None, seed=0):
    ...
    level=logging.DEBUG,
    format='%(asctime)s - %(levelname)s - %(message)s',
    filename=f'{args.plm_type}_{args.plm_size}_{llm_freq}_all_traffic_llm_eval_logs_debug.log',
    filemode='a')
    ...
    # Part 1: Consider both L4S and classic traffic with LLM Policy
    logging.basicConfig(
    level=logging.DEBUG,
    format='%(asctime)s - %(levelname)s - %(message)s',
    filename=f'{args.plm_type}_{args.plm_size}_{llm_freq}_all_traffic_original_eval_logs_debug.log',
    filemode='a')
    ...
    level=logging.DEBUG,
    format='%(asctime)s - %(levelname)s - %(message)s',
    filename=f'{args.plm_type}_{args.plm_size}_{llm_freq}_classic_traffic_llm_eval_logs_debug.log',
    filemode='a')
    ...
    level=logging.DEBUG,
    format='%(asctime)s - %(levelname)s - %(message)s',
    filename=f'{args.plm_type}_{args.plm_size}_{llm_freq}_classic_traffic_original_eval_logs_debug.log',
    filemode='a')
    ...
    level=logging.DEBUG,
    format='%(asctime)s - %(levelname)s - %(message)s',
    filename=f'{args.plm_type}_{args.plm_size}_{llm_freq}_l4s_traffic_llm_eval_logs_debug.log',
    filemode='a')
    ...
    level=logging.DEBUG,
    format='%(asctime)s - %(levelname)s - %(message)s',
    filename=f'{args.plm_type}_{args.plm_size}_{llm_freq}_l4s_traffic_original_eval_logs_debug.log',
    filemode='a')
    ...
    # classic_llm_logs = run_policy(df_classic, traffic_type='classic', use_model_decision=True)

    # # Part 2: Classic traffic without LLM (original, sequential)
    # classic_original_logs = run_policy(df_classic, traffic_type='classic_original', use_model_decision=False)

    # # Part 3: L4S traffic with LLM policy
    # l4s_llm_logs = run_policy(df_l4s, traffic_type='l4s', use_model_decision=True)
    ...
    # # Final save
    # final_logs = {
    #     'classic_traffic_llm': classic_llm_logs,
    #     'classic_traffic_original': classic_original_logs,
    #     'l4s_traffic_llm': l4s_llm_logs,
    #     'l4s_traffic_original': l4s_original_logs
    # }

    # # Loop through the logs and save each to a separate file
    # for log_name, log_data in final_logs.items():
    #     filename = f'{args.plm_type}_{args.plm_size}_{llm_freq}_{log_name}_eval_logs.json'
    #     file_path = os.path.join(base_path, filename)
    #     with open(file_path, 'w') as f:
    #         json.dump(log_data, f, indent=4)
    #     print(f"Saved {log_name} logs to {file_path}")
