...
def run(args):
    ...
    exp_pool_path="./exp_pool.pkl"
    exp_pool_path = "./data/exp_pools/exp_pool_l4s_eval.pkl"
    print("exp_pool_path",exp_pool_path)
    exp_pool = pickle.load(open(exp_pool_path, 'rb'))
    print("min reward",np.min(exp_pool.rewards))
    print("max reward",np.max(exp_pool.rewards))

    # Get the minimum reward and its index
    min_reward = np.min(exp_pool.rewards)
    min_reward_index = exp_pool.rewards.index(min_reward)

    # Get the maximum reward and its index
    max_reward = np.max(exp_pool.rewards)
    max_reward_index = exp_pool.rewards.index(max_reward)

    # Print the states, actions, and rewards for the minimum reward
    print("For minimum reward (index:", min_reward_index, "):")
    print("State:")
    for value in exp_pool.states[min_reward_index]:
        print("{:.6f}".format(value))  # Format the float value to 6 decimal places
    print("Action:", exp_pool.actions[min_reward_index])
    print("Reward:", exp_pool.rewards[min_reward_index])
    print("Reward:{:.6f}".format(min_reward))

    # Print the states, actions, and rewards for the maximum reward
    print("\nFor maximum reward (index:", max_reward_index, "):")
    print("State:")
    for value in exp_pool.states[max_reward_index]:
        print("{:.6f}".format(value))  # Format the float value to 6 decimal places
    print("Action:", exp_pool.actions[max_reward_index])
    print("Reward:", exp_pool.rewards[max_reward_index])

    # print("states:::",exp_pool.states[0])
    ...
    print("exp_dataset_info.max_return",(exp_dataset_info.max_return))
    print(f"min return: {np.min(exp_dataset_info.min_return):.16f}")
    print("args.target_return_scale",(args.target_return_scale))
    print("target_return = exp_dataset_info.max_return * args.target_return_scale",(exp_dataset_info.max_return * args.target_return_scale))
